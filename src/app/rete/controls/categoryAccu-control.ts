...
@Component({
  template: `<label>{{label}}</label><select id="category" name="spesifikasi" (change)="change($event.target.value)">
          <option value="generator" selected="selected">Generator</option>
...
export class CategoryAccuControl extends Control implements AngularControl {
  ...
  constructor(public emitter, public key, public label, readonly = false) {
    ...
    this.props = {
      ...
      value: 0,
      mounted: () => {
        this.setValue(this.getData(key) as any || "generator")
      },label:this.label+" : "
    };
  }
