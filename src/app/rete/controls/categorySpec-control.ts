...
@Component({
  template: `<label>{{label}}</label><select id="category" name="spesifikasi" (change)="change($event.target.value)">
          <option value="fuelcell" selected="selected">Fuel Cell</option>
...
export class CategorySpecControl extends Control implements AngularControl {
  ...
  constructor(public emitter, public key, public label, readonly = false) {
    ...
    this.props = {
      ...
      value: 0,
      mounted: () => {
        this.setValue(this.getData(key) as any || "fuelcell")
      },label:this.label+" : "
    };
  }
